...
@implementation NSObject (HBMember)
...
- (void)enumerateIvarsWithBlock:(HBIvarsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        for (int i = 0; i<outCount; i++) {
            HBIvar *ivar = [[HBIvar alloc] initWithIvar:ivars[i] srcObject:self];
            ivar.srcClass = c;
            block(ivar, stop);
        }
...
- (void)enumerateMethodsWithBlock:(HBMethodsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        // 2.遍历每一个成员变量
        for (int i = 0; i<outCount; i++) {
            HBMethod *method = [[HBMethod alloc] initWithMethod:methods[i] srcObject:self];
            method.srcClass = c;
            block(method, stop);
        }
